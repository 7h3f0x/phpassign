...
$ask="SELECT * FROM results WHERE username='".$username.$num."'";
	$res=$conn->query($ask);
...
if ($option==$result1['correct']) {
	//$q="UPDATE results SET ".$num."='answered-correct'";
	$q="UPDATE results SET status='answered-correct' where userq='".$username.$num."'";
